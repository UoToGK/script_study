...
from db.config import MONGODB_PORT, MONGODB_HOST, MONGODB_NAME
...
class CollectionOperation():
    ...
    def _update_one(self, key, data):
        ...
        result = self.table.find_one({'aweme_id_create_time': key})
        # 数据存在可以更新
        if type(result) is dict:
            self.table.update_one({'aweme_id_create_time':key}, {"$set": data})
            op_result = 'UPDATE'
        # 数据不存在调用插入
        else:
            self._insert_one(data)
...
if __name__ == "__main__":
    data = {'mp4': 0, 'create_time': 8, 'aweme_id': '643962374688604160', 'id': '6b6934a83fa4385ed4ec53f987e07b5f'}
    col = CollectionOperation('douyin')
    col.insert('id', data)
